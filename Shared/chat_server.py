...
class ChatHandler(http.server.BaseHTTPRequestHandler):
    """Handles all HTTP requests for the drtlemon AI Chat."""
    ...
    def _scan_gguf(self):
        ...
        request_context = self._build_request_context("/api/scan-gguf")
        try:
            scan_dirs = [
            ...
            _log_event(logging.ERROR, "GGUF scan failed", request_context=request_context, exc_info=True)
            self.send_response(500)
            self._cors_headers()
            self.end_headers()
            self.wfile.write(json.dumps({"error": str(e)}).encode())
...
def main():
    ...
    print()
    print("=" * 55)
    print("  drtlemon AI — Chat Server")
    print("  Daramola Olasupo · StoneWeb InFOMIX Enterprises")
    print("=" * 55)
    print()
